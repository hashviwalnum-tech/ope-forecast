...
- **Guided product tour (coachmarks) — web AND mobile.** A friendly, welcoming walkthrough separate from the setup wizard. Flow: a warm welcome that explains what Ope is FOR (in plain, friendly language — not jargon), then steps through the features ONE BY ONE, each with an arrow/highlight pointing at the actual UI element and a short, friendly explanation of what it does. Requirements:
  - **Goes DEEP, not just top-level:** don't only introduce the main sections (Forecast, Insights, Manage, etc.) — go INTO each section and explain **every feature within it**, one at a time. This includes **ALL settings options**, the **dark-mode toggle on the dashboard**, and other controls — be thorough, cover everything a user can do.
  - **Suggest DARK MODE and FRIENDLY MODE at the START of the tour** — near the beginning, offer the user to enable dark mode and/or extra-friendly language, so they set their comfort preferences before the walkthrough proceeds.
  - **Three buttons on each step:** (1) **Skip all** — exits the entire tour; (2) **Skip [section name]** — skips the rest of the CURRENT section, labeled with that section's real name (e.g. "Skip History" when in the history area), not a generic word; (3) **Next** — advance to the next explanation.
  - **Hebrew support with an in-tour language toggle** — the tour is fully translated (Hebrew + English), AND the user can toggle the language ON THE TOUR MESSAGE ITSELF (flip language mid-tour).
  - **Auto-launches for new users** (after setup, including for me), **re-launchable any time from Settings**.
  - **Handles the empty state gracefully** — for a new user with no data, steps explain what WILL appear, never pointing at confusing empty charts.
  - **Web and mobile each get their own tour** (different layouts) sharing the same friendly script/tone.
  - Friendly, encouraging tone throughout.
- **Extra-friendly language mode (optional simplicity toggle).** Some owners find even normal labels too technical. A **Settings toggle** turns on extra-simple, plain wording for the parts that are **actually technical/confusing** — ordering/stock terms (e.g. "buffer"), stats, forecast language — while leaving already-clear text (buttons like Save/Next) alone (don't duplicate text that needs no simplifying). Both languages (Hebrew + English) get the simple variant. **New-user suggestion:** gently suggest enabling it to new users in a way that does NOT hinder or interrupt the tutorial flow (e.g. a small non-blocking prompt, or offered alongside settings) — never a popup that blocks the tour. Off by default; normal language is standard. **Built BEFORE the tutorial so the tutorial accounts for it** (the tour explains the toggle and its own text respects the active mode).
- **Settings access — a gear icon on the dashboard.** Settings live behind a **standard settings gear icon (a plain cog — NOT the app's logo/brand mark) placed on the dashboard** itself, easily reachable. **Remove settings from the "Manage" section** — it should no longer live there; the dashboard gear is the single entry point. (Note: this gear is the conventional settings cog and is intentionally distinct from the app's gear-O logo, which serves a different purpose.)
...
│   │       ├── queueing.py
│   │       ├── ensemble.py
│   │       ├── planning.py    # the §7.5 toolbox — see the note there
│   │       └── currency.py    # ISO 4217 codes and their precision (§5)
│   ├── tests/engine/         # known-answer tests for every formula (section 8)
│   │   └── parity/           # proves a formula moved here still returns the
│   │                         # same numbers as the version it replaced
│   └── requirements.txt
...
### Entities
- **Business** — id, name, settings (opening days/hours, default lead time, target service level, average service time per customer for staffing, **currency**). One row in Phase 1; FK to user in Phase 2. **Opening days/hours must be editable in a settings screen** — and the forecasting engine must use them: closed days are excluded from forecasting entirely (not treated as zero-customer days), and hourly features only consider open hours.
  - **CURRENCY (per business, ISO 4217) — DONE.** Every business picks its own, during onboarding and editable in settings, from the list at `GET /currencies` (`engine/currency.py` is the single source of truth, so the codes the settings endpoint validates are the ones the picker offers). Chosen from a locale guess but **never applied without the owner confirming** — a wrong currency accepted silently mislabels every price typed afterwards. An unrecognised code is **refused, not defaulted**.
    - **Every money figure everywhere goes through it** — prices, regulars' spend and CLV, ad/event cost, the planning toolbox. No screen may name a currency: a guard test fails the build if one does.
    - **Never assume two decimal places.** The yen and won have none (¥1200, not ¥1200.00), the Gulf dinars have three. Formatting passes no fraction-digit count of its own and lets the runtime use the currency's own precision; money inputs step by the currency's smallest unit, so a form cannot offer to type ¥1200.50.
    - **Amounts stay currency-agnostic in storage** — plain decimal numbers, never "cents", so nothing in the database depends on a currency's precision and changing currency needs no migration.
    - Number *formatting* follows the language the owner picked in Ope, not the browser's.
...
- **OrderRecord** *(ordering lifecycle — workflow-embedding feature)* — id, business_id, product_id, ordered_date, quantity, expected_arrival_date (auto = ordered_date + product.lead_time_days), status. The owner logs **"I ordered X units"** via a button; the app records it and **assumes arrival after the product's lead time** (no second confirmation — expected_arrival_date auto-set), at which point projected stock increases by the quantity. The order is **cancellable/editable until closing hours** that day (locks after close, same forgiving pattern as day-records/regulars). Also track **product creation date** (when the product was added) so its history has a start point.
  ...
  - **ALL ordering/stock language must be PLAIN, friendly labels (same info, no jargon) — applied everywhere ordering/stock appears** (reorder window, product-status window, ordering screens, stock messages, reminders). A simple shop owner shouldn't decode inventory terms. Rephrase the metric names into everyday what-to-do language while keeping the same numbers, e.g.: "Reorder below: 73" → "Order more when you drop below 73"; "Safety buffer: 51" → "Keep at least 51 as backup"; "~22/day" → "You use about 22 a day"; "restock in 1d" → "Takes about 1 day to arrive"; "You're good" stays. Describe the ACTION and WHY in everyday words, not the inventory-metric name. Consistent wording across every ordering/stock surface. Fully translatable (Hebrew + English).
...
These are clearly later-tier and must not bloat or intimidate the core experience. **Each tool must clearly EXPLAIN, in plain language, what it actually checks/does** (a short description per tool so a non-expert understands its purpose), and all of it must be **fully translated to Hebrew** — *verified done: all 80 toolbox strings are present in all 15 languages. An earlier report that 5 languages were missing them was a fault in the checking script, not in the translations; a key can share a line with another in `i18n.ts`, and the check only looked at the start of each line.* **Calmer ordering wording:** any "how does this ordering decision feel" / confidence prompt must be **worded gently and placed at the BOTTOM** of the ordering view, not upfront — the current phrasing/placement can feel alarming to owners. Reassure, don't interrogate.

**Where the maths lives — `engine/planning.py`, behind `POST /planning/{decision,framing,budget}` (DONE).** These calculations ran in the browser and were moved, per §4 and §11: a client that calculates is a client the mobile app has to reimplement and then keep in step by hand. Three of the four tools have maths; the fourth ("my action list") is a device-local checklist with nothing to compute, so it has no endpoint. Two things worth keeping in mind if they are ever changed:
- **The budget tool must stay an exact solver, not a greedy one.** Sorting by profit-per-cost and buying down the list is only optimal when units are divisible. On whole units it can be badly wrong — with 100, item A at 60 returning 65 and item B at 50 returning 50, greedy buys one A for 65 while two Bs return 100. It is a bounded knapsack now, with a test pinning that case.
- **A formula moved into the engine gets a parity check, not a promise.** `tests/engine/parity/` keeps a frozen copy of the implementation being replaced, runs both over the same inputs, and fails if any value differs — with a further test that the frozen copy has not been edited to agree. Reuse this whenever logic moves.
