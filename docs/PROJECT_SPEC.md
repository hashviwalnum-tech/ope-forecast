...
- **Design language — calm, comfortable, inviting:**
  ...
  - **Localization completeness:** ALL visible text including chart labels, axis labels, card titles, button text, tooltips, and empty-state messages must be translated when Hebrew is selected. Untranslated strings are a bug. Right-to-left layout where needed.
...
- **Regular** *(separate entity & data store — NOT in past-data/demand history)* — id, business_id, name, **avg_spend_per_visit** (what they typically pay per visit), derived **CLV** (= avg_spend × expected visit frequency × expected lifespan — auto-computed), optional notes. Edited on a dedicated screen (enter name + avg spend → auto-CLV). A separate **"Record a regular"** quick action logs a visit with the **amount paid that visit** (defaults to avg_spend, editable). Regulars are logically **max once per day per regular** (if the same regular is recorded twice in one day, warn or merge). **Recording a regular is explicitly ALLOWED during open hours** — it's a point event (they came right now), not a daily total, so the entry-timing rule on sales does NOT apply. Regulars live in their own data; they never enter DayRecord/SaleEvent demand history.
...
- **Show progress toward reliability.** Friendly "log about N more days for reliable forecasts" messaging.
