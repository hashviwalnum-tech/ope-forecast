...
- **Staffing per shift** — using the hourly arrival rate (λ) and average service time, find the smallest number of servers `c` that keeps utilization below a safe threshold (and wait under target): "for the 5–6pm rush, schedule 3 people." Depends on transaction capture for hourly λ.
  ...
  - **Ignore out-of-opening-hours entries.** Any customers/sales recorded in hours outside the business's opening hours must be ignored by the forecasting and staffing math (they shouldn't exist; don't let a stray out-of-hours number pollute the model), even if such an entry was somehow marked.
...
- **Data consistency rules (enforce the math; the daily total is the source of truth):**
  - **Customer total must reconcile with the sum of hours (REPEATEDLY NOT IMPLEMENTED — must actually work this time, with a test).** When a day has hourly entries, the app must compare the sum of typed hours against the daily customer total. If they don't match, **notify the user** and offer the choice: "treat the untyped hours as unknown" — meaning the typed hours stay as real hourly data, and the difference (total − typed hours) is counted in the day total as unattributed/unknown hours. Known hours must never exceed the total (warn if they do). The hours chart and the daily total must never silently disagree without the user being told.
