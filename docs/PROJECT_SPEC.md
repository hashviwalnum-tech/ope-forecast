...
- **Guided product tour (coachmarks) — web AND mobile.** A friendly, welcoming walkthrough separate from the setup wizard. Flow: a warm welcome that explains what Ope is FOR (in plain, friendly language — not jargon), then steps through the features ONE BY ONE, each with an arrow/highlight pointing at the actual UI element and a short, friendly explanation of what it does. Requirements:
  - **Every major feature gets a step**, walked one at a time. Each step has **"skip this step"** (advance/next) AND a **"skip everything"** (exit the whole tour) — so a user is never trapped; length is safe because any step is skippable.
  - **Auto-launches for new users** (after the setup wizard, including for me so I can see it), and is **re-launchable any time from Settings** ("Replay tour" / "Show tutorial again").
  - **Handles the empty state gracefully** — for a brand-new user with no data yet, steps explain what WILL appear ("your forecast shows here once you log a few days"), never pointing at a broken-looking empty chart confusingly.
  - **Web and mobile each get their own tour** (different layouts/navigation) sharing the same friendly script/tone. Fully translatable (Hebrew + English).
  - Friendly, encouraging tone throughout — welcoming someone into Ope, not lecturing them.
