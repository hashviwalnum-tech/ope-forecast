...
**Later (only once ≥1 year of history exists) — SEASONALITY.** Detect recurring seasonal patterns (e.g. December busier, summer slump) from the *full* history as monthly/seasonal indices, and fold them in. This is a data-dependent future feature, not buildable or testable now; flag it but don't depend on it. It must degrade gracefully (do nothing) until enough history exists. **Monthly comparison should include same-month-last-year** (e.g. this June vs last June) once a year of data exists — a useful year-over-year view for the owner.
...
- **OrderRecord** *(ordering lifecycle — workflow-embedding feature)* — id, business_id, product_id, ordered_date, quantity, expected_arrival_date (auto = ordered_date + product.lead_time_days), status. The owner logs **"I ordered X units"** via a button; the app records it and **assumes arrival after the product's lead time** (no second confirmation — expected_arrival_date auto-set), at which point projected stock increases by the quantity. The order is **cancellable/editable until closing hours** that day (locks after close, same forgiving pattern as day-records/regulars). Also track **product creation date** (when the product was added) so its history has a start point.
  - **Projected stock tracking:** the app maintains projected stock over time = starting/current stock − forecast daily sales + arrivals from logged OrderRecords. This is the engine behind the warning below.
  - **"You didn't order" warning — fires ONLY when projected stock is about to run out** (when the owner is "supposed to be over"/out of stock), not on every recommendation. The warning is **dismissable** ("leave it be") so the owner can override (they may know something the app doesn't). When dismissed, don't nag again for that cycle.
  - **Order confirmation button:** prominent "I ordered this" action on a product's ordering recommendation, capturing the quantity ordered; this creates the OrderRecord and starts the arrival/stock projection.
