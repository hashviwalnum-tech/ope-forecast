...
export default function LogDayForm({ onSaved }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-sm">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1.5 dark:text-slate-200" htmlFor={`${fieldId}-f1`}>
        ...
        />
        {validationError && (
          <p role="alert" className="text-sm text-red-600 mt-1">{validationError}</p>
        )}
      </div>
